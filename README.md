...
- 핵심: **쓰기 DB의 변경을 이벤트로 표준화(Outbox)** 하고, **읽기 모델(Projection)을 별도로 유지**해  
  검색/집계 성능과 확장성을 확보합니다.
- 목표: “조인/집계 때문에 느려지는 화면”을 **Projection 조회**로 바꾸고, 변경은 이벤트로 **증분 갱신**합니다.

✅ **Outbox 기반 CDC 파이프라인**: 도메인 변경을 이벤트로 캡처 → Relay/Consumer로 Projection **증분 동기화**  
✅ **운영 내구성**: Relay 동시성(락 기반 선점) + Consumer 멱등(UPSERT) + 재시도/격리로 중복/재처리에도 일관성 유지  
...
> 확장 경로(선택): Outbox → Relay → (Kafka 같은 스트림) → Consumer 로 자연스럽게 확장 가능  
> (MVP는 “Outbox + Relay/Consumer”만으로도 운영 패턴을 증명하는 데 초점)
...
- Write/Read 모델을 분리하고, 원본 변경을 이벤트로 캡처해 `product_search` 등 Projection을 **증분 갱신**
- 화면/검색 API를 Projection 조회로 전환 → **조인 없는 조회 구조**로 읽기 성능·확장성 개선

### 2) Outbox 패턴으로 이벤트 일관성 보장
- 도메인 변경과 `outbox_events` 기록을 **동일 트랜잭션으로 커밋**
- Relay는 상태 머신(NEW → PROCESSING → SENT/FAILED) 기반으로 발행하여
  **“DB는 됐는데 이벤트가 없음”** 같은 불일치 리스크를 줄입니다.

### 3) Relay 동시성 안전 처리 + 운영 내구성
- 다중 Relay 환경에서 `SELECT ... FOR UPDATE SKIP LOCKED`로 **작업 선점(락 기반 할당)**  
  → 중복 처리/경쟁 조건 방지
- 재시도 횟수/실패 사유 기록, 오래된 PROCESSING을 되돌리는 reaper(선택) 등
  운영자가 “막힌 이벤트”를 복구 가능한 구조로 설계

### 4) 멱등 Consumer + Commit 규칙
- Consumer는 Projection을 **UPSERT**로 갱신해 중복/재처리에도 결과가 동일하도록 보장(멱등)
- DB 반영 성공 후에만 ack/commit(또는 offset commit)  
  → 실패 시 재처리로 자동 복구 가능

### 5) Restock Agent (recommend-only)
- `/ai/restock/agent`에서 판매량/임계값 기반 추천을
  **요약(needCount/totalIn/byReason) · 정렬(topNeeds) · trace(근거)** 형태로 제공합니다.
- `explain_llm=true` 옵션 시 LLM으로 **설명/주의사항/품목별 1줄 요약** 생성  
  (키 없으면 규칙 기반 fallback)

### 6) (옵션) CSV 기반 스냅샷 동기화 + 이벤트 재사용
- CSV 업로드를 “스냅샷 반영”으로 시작하고, 반영 과정에서 생성되는 변경 이벤트(Outbox)를
  **알림/검색/추천에 재사용**할 수 있게 설계할 수 있습니다.
- 초기에는 서버 내부 폴링/SSE로 단순화하고, 사용량 증가 시 Redis Pub/Sub 또는 스트림으로 전환하는 확장 경로를 염두에 둡니다.
...
- Redis (cache/aux)
- (선택) Kafka 같은 스트림으로 확장 가능 구조 (Outbox → Relay → Stream → Consumer)
...
- `outbox_events`: CDC Outbox 이벤트 테이블 (status/retries/last_error/payload 등)
- `product_search` 등: Projection(Read Model)
- `restock_idempotency`: 실행형 엔드포인트의 멱등/재사용(운영 내구성)
...
## Reliability Notes (장애/복구 시나리오)

- Relay 중단 → Outbox(NEW)가 DB에 누적 → Relay 재시작 시 이어서 처리
- Consumer 중단 → 이벤트 누적 → Consumer 재시작 시 Projection 추격
...
<!-- 예: docs/stackops-cdc-pipeline.png -->
