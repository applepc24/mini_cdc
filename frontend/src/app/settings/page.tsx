    ...
    <AppLayout>
      ...
      >
        <motion.div variants={itemVariants}>
          ...
          <p className="text-muted-foreground">StockPulse 환경을 맞춤 설정하세요</p>
        </motion.div>
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <input
                  ...
                  onChange={e => setLocalSettings(prev => ({ ...prev, threshold: Number(e.target.value) }))}
            ...
            <div>
              ...
              <div className="flex gap-2">
                {itemsPerPageOptions.map(option => (
                  <button
                    key={option}
                    onClick={() => setLocalSettings(prev => ({ ...prev, itemsPerPage: option }))}
                    className={cn(
        ...
        <motion.div
          ...
          className="bg-card rounded-xl border border-border p-6"
        >
          <h2 className="text-lg font-semibold text-foreground mb-4">키보드 단축키</h2>
          
          <div className="space-y-3">
            {[
              { keys: ['/', 'or', 'Ctrl', 'K'], description: '검색 / 명령 팔레트 열기' },
              ...
              <div key={index} className="flex items-center justify-between py-2">
                ...
                <div className="flex items-center gap-1">
                  {shortcut.keys.map((key, i) => (
                    key === 'or' ? (
                      <span key={i} className="text-xs text-muted-foreground mx-1">or</span>
                    ) : (
                      <kbd
                        key={i}
